...
FRAME_THRESHOLD = 10
RATIO_THRESHOLD_LB = 0.40
RATIO_THRESHOLD_UB = 0.60
COUNTER = 0
TRIGGERED = 0
MAX = 0
MIN = 100
...
vs.stream.set(cv2.CAP_PROP_FPS, 20)
# pprint(vs.stream)
time.sleep(2.0)
...
while True:
    ...
    if TRIGGERED >= 2:
        print("POST SENT")
        resp = requests.post("http://172.16.10.9:3000/flip", json={"flip":True} )
        pprint(resp)
        # if (resp.confirm == True):
        resp = requests.get("http://172.16.10.9:3000/flip")
